...
export class ArchivedVideo implements OnInit {
    ...
    view_video(camera_id: number){
        this.router.navigate(['/security-agent/video/view'],{ queryParams: { camera_id : camera_id, media_type: 'video' } });
    }
                ...
                data => {
                    ...
                    if(this.response.status == 'error'){
                        ...
                        for(var s=0; s < this.videos.length ; s++) {
                            ...
                            this.startCamera.camera_id = this.videos[s].camera_id;
    ...
    delete_camera(){
        console.log(this.gray_video_id);
        this.videoService.remove(this.gray_video_id)
            .subscribe(
                data => {
                    this.response = data;
                    if(this.response.status == 'error'){
                        ...
                        document.getElementById("GRAY_"+this.gray_video_id).remove();
                        const dialog =   this.modal.alert()
                                        .size('sm')
                                        .body(this.response.message)
                                        .open();
