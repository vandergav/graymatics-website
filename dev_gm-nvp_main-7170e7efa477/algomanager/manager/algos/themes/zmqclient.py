...
logger.setup_logger("zmqclient",'zmqclient.log')
logger_1 = logging.getLogger("zmqclient")
...
class SerializingSocket(zmq.Socket):
	...
	def send_array(self, A, arrayname="NoName",flags=0, copy=True, track=True):
		...
		logger_1.debug({"insend array" : md})
		return self.send(A, flags, copy=copy, track=track)

	def recv_array(self, flags=0, copy=True, track=False):
		"""recv a numpy array, including arrayname, dtype and shape"""
		logger_1.debug({"in rec_array " : track})
		md = self.recv_json(flags=flags)
		msg = self.recv(flags=flags, copy=copy, track=track)
		A = np.frombuffer(msg, dtype=md['dtype'])
		logger_1.debug({"rec array ending " : A})
...
class zmqConnect():
	...
	'''
	def __init__(self, connect_to="tcp://localhost:7050"):
		...
		logger_1.debug({"in ZmqCOnnect " : "before init"})
		self.zmq_context = SerializingContext()
		self.zmq_socket = self.zmq_context.socket(zmq.REQ)
		self.zmq_socket.connect(connect_to)
		logger_1.debug({"zmqConnect init done" :self.zmq_socket})
	...
	def send(self, arrayname, array):
		...
		if array.flags['C_CONTIGUOUS']:
			...
			self.zmq_socket.send_array(array, arrayname, copy=False)
		logger_1.debug({"after zmq socket sendarray" :"sendarray"})
		message = self.zmq_socket.recv()
		logger_1.debug({"zmq finish" : message})
