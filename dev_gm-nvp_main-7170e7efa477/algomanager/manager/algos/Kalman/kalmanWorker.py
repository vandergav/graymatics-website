...
class KalmanWorker:
        ...
        def initParams(self, roi):
                try:
                        if "image" not in roi:
                                roi = {"image":{"width":640, "height":480}}
                        h = roi["image"]["height"]
                        w = roi["image"]["width"]
                        #if "roi" in roi:
			if len(roi) > 1:
                                print "roi", roi['roi']
				pt1 = (roi[1][0][0], roi[1][0][1])
				pt2 = (roi[1][1][0], roi[1][1][1])
				self.counting_line_points1 = pt1
				self.counting_line_points2 = pt2
                        else:
				self.counting_line_points1 = (0, h/2) 
				self.counting_line_points2 = (w, h/2) 
