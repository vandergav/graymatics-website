...
var q1 = async.queue(function(task, callback) {
	...
	if( typeof task.response != 'undefined' &&
		...
		if(	typeof camera_list != 'undefined' &&  
			typeof camera_list[task.response.cam_id] != 'undefined' &&  
			camera_list[task.response.cam_id].total < total) {
			logger.info("== Rule: total count should be greater than previous count(VC)");
			// to validate for the same frame results
			camera_list[task.response.cam_id].total = total;
			...
			if(total >= camera_list[task.response.cam_id].counter*camera_list[task.response.cam_id].thresh_hold){
				...
				logger.info("== Thresh Hold: for every point (VC)");
			...
			if( plate_flag == true && 
				...
				plate_flag = true;	
				// Get black/white list vehicle numbers
				// rest.get(config.url+'/vehicle_recognition/list_name/:user_id/:list_name').on('complete', function(data) {
				// 	logger.info(data); // auto convert to object
				// });
	...
	if( typeof task.response != 'undefined' &&
		...
		typeof res.FACE_RECOGNTION != 'undefined') {
			var person_objects = "";
			// new notifcation id
			logger.info("Face Recognition..",res.FACE_RECOGNTION.face_url);
			var notification_id = mainHelper.randomToken();
			task.response.notification_type = "INTRUSION";
			task.response.notification_id = notification_id;
			...
			if(res.FACE_RECOGNTION.length > 0) {
				...
				// }
				rest.get(config.url+'/face_recognition_details/22/'+res.FACE_RECOGNTION.face_url).on('complete', function(response) {
					logger.info("FaceDetials::",response); 
					if( response.subject_type == 'Blacklist') {
						person_objects += person_objects + "\n Blacklist Peson detected." + response.gender + ", " + response.age + ", " + response.ethnicity;
						/**
