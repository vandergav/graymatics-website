...
basic_notification.prototype.getNotificationCount = function(callback) {
    ...
    var SQLQUERY =	db.mysql.query("SELECT COUNT(*) AS count FROM `basic_notification` WHERE `user_id` = ? AND `type` = ? AND `start_time` >= ?",
                        [self.user_id,self.type,self.start_time],
                        function(err, result, fields) {
                        if(err){
                            logger.error(SQLQUERY.sql+":"+err.message)
                        }else{
...
basic_notification.prototype.getActionNotificationCount = function(callback) {
    ...
    logger.info("Model Parameters : req.params: ", self);
    var SQLQUERY =	db.mysql.query("SELECT COUNT(*) AS count FROM `basic_notification` WHERE `user_id` = ? AND `type` = ? AND `start_time` >= ? AND `status` = ?",
                        [self.user_id,self.type,self.start_time,self.status],
                        function(err, result, fields) {
                        if(err){
                            logger.error(SQLQUERY.sql+":"+err.message)
                        }else{
