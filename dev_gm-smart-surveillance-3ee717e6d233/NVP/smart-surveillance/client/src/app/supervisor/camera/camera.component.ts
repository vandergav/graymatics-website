...
export class AddComponent implements OnInit {
        ...
        .subscribe(params => {
            ...
            if ( typeof params['algo'] != 'undefined' && params['algo'].length > 0 ) {
                for ( var a=0; a < params['algo'].length ; a++ ) {
                    ...
                    if( key == 'speed_detection' ) this.camera.vehicleSpeed = true;
                    if( key == 'congestion_detection' ) this.camera.intrusionDetection = true;
                }
            }
            if(params['_id'])
    ...
    add(model: Camera, isValid: boolean) {
        ...
        if(model.vehicleSpeed) this.camera.algo.push("speed_detection");
        if(model.intrusionDetection) this.camera.algo.push("congestion_detection");
