...
export class UploadedVideo implements OnInit {
        ...
        .subscribe(params => {
            ...
            if ( typeof params['algo'] != 'undefined' && params['algo'].length > 0 ) {
                for ( var a=0; a < params['algo'].length ; a++ ) {
                    ...
                    if( key == 'speed_detection' ) this.video.vehicleSpeed = true;
                    if( key == 'congestion_detection' ) this.video.intrusionDetection = true;
                }
            }
            if(params['_id'])
    ...
    add(model: Video, isValid: boolean) {
        ...
        if(model.vehicleSpeed) this.video.algo.push("speed_detection");
        if(model.intrusionDetection) this.video.algo.push("congestion_detection");
