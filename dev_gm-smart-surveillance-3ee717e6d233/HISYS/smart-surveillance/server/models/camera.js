...
camera.prototype.save = function(callback) {
	var self = this
	// for now static data
	var url = self.source.split("/");
	self.name = url[url.length - 1];
	self.protocol = 'RTSP';
		...
		var SQLQUERY =	db.mysql.query("INSERT INTO `camera` SET ?",
							...
							function(err, result, fields) {
							    if (!err) {
							    	...
							    	self.camera_id = result.insertId
							    }else if(err){
...
camera.prototype.getUserCameras = function(user_id,callback) {
	...
	var SQLQUERY =	db.mysql.query("SELECT * FROM `camera` WHERE `user_id` = ? ORDER BY camera_id DESC LIMIT 50",
						[user_id],
						function(err, result, fields) {
						  if(err){
							  logger.error(SQLQUERY.sql+":"+err.message)
